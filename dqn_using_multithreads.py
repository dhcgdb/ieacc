...
class NdnParam(Structure):
    ...
    _fields_ = [
        ...
        ('avgDelay', c_double),
        ('DataNum', c_uint32),
        ('InflightNum', c_uint32),
        ('NackNum', c_uint32),
        ('TimeoutNum', c_uint32),
        ('congesLevelSum', c_uint32),
        ('dataSizeSum', c_uint32),
    ]
...
def ndngetstate(var):
    ...
    acks = data.env.DataNum
    Rloss = data.env.TimeoutNum
...
def actualrun(var, index):
    logfile = open("./log/spec/dqn_pylog" + str(index) + ".log", "w")
    dqn = DQN()
    j = 0
    s = ndngetstate(var)
    while (True):
