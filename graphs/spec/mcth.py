...
styleset = ['-.', ':', '--', 'dotted']
pointset = ['o', 'v', 'D', '+']
nameset = ['consumer1', 'consumer2', 'app3', 'app4']
suffixset = ['0', '1']#, '2']
titleset = ['IEACC', 'ICP', 'ECP', 'DRL-CCP']
itemset = ['ddpg', 'aimd', 'ecp', 'dqn']
legendpos = ['lower right', 'lower right', 'lower right', 'lower right']
#BWs = [50, 70, 60, 40]
#TPs = [0, 20, 40, 60, 80]
BWs = [50]
TPs = [0, 75]
...
for item in itemset:
    ...
    if (item == 'ddpg'):
        for suffix in suffixset:
            ...
            y0.append(0)
            #plt.subplot(subfig)
            plt.title(titleset[subfig - 221], fontdict={"size": 16})
            plt.plot(x0,
                     y0,
            ...
            plt.legend(loc=legendpos[pos], prop={"size": 16})
    else:
        for suffix in suffixset:
    ...
    plt.savefig('./4in1-split'+item+'.pdf')
#plt.savefig('./4in1.2.pdf')
