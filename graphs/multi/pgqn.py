...
for suffix in suffixset:
    xc0, yc0 = [], []
    with open('/root/ndn/proj-sep/log/multiport/' + suffix + '-c0-drop-91.log',
              'r') as filep:
    ...
    file11 = '/root/ndn/proj-sep/log/multiport/' + suffix + '-c1-drop-91.log'
    ...
    #              ])
    plt.plot(x0, y0, label=suffixset[subfig - 221])
    plt.grid(which='both', axis="y", linestyle='-.')
    plt.ylim((0, 110))
    plt.xlabel("time (s)")
    plt.ylabel("Data transmission rate (Mbps)")
plt.legend()
plt.savefig('./pgqn.png')
