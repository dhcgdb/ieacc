...
namespace nfd
{
    namespace fw
    {
        ...
        BestRouteStrategy2WithECP::BestRouteStrategy2WithECP(Forwarder &forwarder, const Name &name)
            : Strategy(forwarder), ProcessNackTraits(this), m_retxSuppression(RETX_SUPPRESSION_INITIAL, RetxSuppressionExponential::DEFAULT_MULTIPLIER, RETX_SUPPRESSION_MAX), forwarderAcc(forwarder)
        {
            ...
            {
                NDN_THROW(std::invalid_argument("BestRouteStrategy2WithOutFaceWnd does not support version " + to_string(*parsed.version)));
            }
        ...
        void BestRouteStrategy2WithECP::afterReceiveInterest(const Interest &interest, const FaceEndpoint &ingress,
        ...
        void BestRouteStrategy2WithECP::afterReceiveNack(const lp::Nack &nack, const FaceEndpoint &ingress,
                                                         const shared_ptr<pit::Entry> &pitEntry)
        {
        ...
        void BestRouteStrategy2WithECP::beforeSatisfyInterest(const Data &data, const FaceEndpoint &ingress,
                                                              const shared_ptr<pit::Entry> &pitEntry)
        {
        ...
        void BestRouteStrategy2WithECP::afterContentStoreHit(const Data &data, const FaceEndpoint &ingress,
                                                             const shared_ptr<pit::Entry> &pitEntry)
        {
